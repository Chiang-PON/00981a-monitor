...
def _row_to_item(row: pd.Series, c_name: str, code: str, diff: int, amount: float, w_diff: float = 0.0) -> dict:
    return {
        ...
        "weight": float(row.get("weight", 0) or 0) if "weight" in row.index else 0.0,
        "w_diff": w_diff,
    }
        ...
        for i in range(1, len(files)):
            ...
            try:
                ...
                for c in (curr_codes - prev_codes):
                    ...
                    new_buy.append(_row_to_item(row, c_name, c, shares, amount_10k, float(row.get("weight", 0) or 0)))
                ...
                for c in common_codes:
                    row_curr = curr_idx.loc[c]
                    row_prev = prev_idx.loc[c]
                    shares_curr = int(row_curr["shares"] / 1000)
                    shares_prev = int(row_prev["shares"] / 1000)
                    diff = shares_curr - shares_prev
                    if diff == 0:
                        continue
                    weight_curr = float(row_curr.get("weight", 0) or 0)
                    weight_prev = float(row_prev.get("weight", 0) or 0)
                    w_diff = weight_curr - weight_prev
                    price_curr = float(row_curr.get("price", 0) or 0)
